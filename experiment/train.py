...
def train_model(
    ...
    dataset = DentalDataset(dir_img, dir_mask, img_scale)
    ...
    # cls_weight = dataset.compute_class_weight().to(device)
    cls_weight = None
    logging.info(f"Class weight: {cls_weight}")
            ...
            for batch in train_loader:
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        evals = evaluate(model, val_loader, device, amp)
                        dice = evals['Scores'].loc['IoU', 'Macro-avg']
                        scheduler.step(dice)
