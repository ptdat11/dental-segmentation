...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, mask_suffix: str = ''):
        ...
        if not self.ids:
            raise RuntimeError(f'No input file found in {
                               images_dir}, make sure you put your images there')
    ...
    def __getitem__(self, idx):
        ...
        assert len(img_file) == 1, f'Either no image or multiple images found for the ID {
            name}: {img_file}'
        assert len(mask_file) == 1, f'Either no mask or multiple masks found for the ID {
            name}: {mask_file}'
    ...
    def compute_class_weight(self):
        cls_freq = torch.ones(15)
        n_cls = cls_freq.size(0)
        for name in tqdm(self.ids, total=len(self.ids), desc="Computing class weight"):
            mask_file = list(self.mask_dir.glob(
                name + self.mask_suffix + '.*'))

            assert len(mask_file) == 1, f'Either no mask or multiple masks found for the ID {
                name}: {mask_file}'
            mask = array(load_image(mask_file[0]))

            freq = torch.bincount(torch.from_numpy(
                (mask)).ravel(), minlength=n_cls)
            cls_freq += freq

        cls_weight = cls_freq.sum() / (n_cls * cls_freq)
        return cls_weight
